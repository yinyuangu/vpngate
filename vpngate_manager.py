...
def row_to_node(row: dict[str, str], config_text: str) -> dict[str, Any] | None:
    ...
    country_zh = vpn_utils.translate_country(country_long, country_short)
    return {
        "id": node_id,
        "country": country_zh,
...
def kill_existing_openvpn_processes() -> None:
    ...
    try:
        ...
        print("[Cleanup] Terminated existing VPNgate OpenVPN processes.", flush=True)
    except Exception as e:
...
<head>
  ...
  <style>
    ...
    .filter-menu-btn {
      ...
      white-space: nowrap;
      position: relative;
      box-shadow: inset 0 1px 0 rgba(255,255,255,0.045), 0 8px 18px rgba(0,0,0,0.08);
    }
    ...
    @media (min-width: 1600px) {
      header {
        padding-left: calc((100vw - 1360px) / 2 + 18px);
        padding-right: calc((100vw - 1360px) / 2 + 18px);
      }
    }
...
function countryOptionLabel(country) {
  ...
  return `${flag ? `${flag} ` : ""}${translateCountry(country, sample && sample.country_short)}`;
}
...
function nodeCountryLabel(node) {
  ...
  return `${flag ? `${flag} ` : ""}${translateCountry(node.country, node.country_short)}`;
}
...
function countryLockLabel(ch) {
  ...
  const flag = countryFlag(sample && sample.country_short);
  return `国家锁定: ${flag ? `${flag} ` : ""}${translateCountry(ch.country_lock, sample && sample.country_short)}`;
}
